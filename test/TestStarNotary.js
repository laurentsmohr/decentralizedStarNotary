...
it('can add the star name and star symbol properly', async () => {
  let instance = await StarNotary.deployed();
  let starId = 6;
  await instance.createStar('star boy', starId, { from: accounts[1] });
...
it('lets 2 users exchange stars', async () => {
  ...
  await instance.createStar('Star1', 7, { from: user1 });
  await instance.createStar('Star2', 8, { from: user2 });
  await instance.exchangeStars(7, 8, { from: user1 });
  const ownerOfStar1 = await instance.ownerOf(7);
  const ownerOfStar2 = await instance.ownerOf(8);
...
it('lookUptokenIdToStarInfo test', async () => {
  let instance = await StarNotary.deployed();
  let starId = 10;
  await instance.createStar('neptun', starId, { from: accounts[1] });
  const name = await instance.lookUptokenIdToStarInfo(starId);
  assert.equal(name, 'neptun');
});
